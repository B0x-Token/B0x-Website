...
contract B0xWebsiteFrontend is IDecentralizedApp {
    ...
    function setChunk(string calldata path, uint256 index, bytes calldata data) external onlyOwner {
        ...
        // Set unconditionally (not just on the push branch above): after a
        // clearFile() batch a path can sit unregistered with c.length > 0,
        // so a re-upload landing back on the overwrite branch must still
        // re-register it, not just the branch that grows the array.
        registered[path] = true;
    }

    /// @notice Pops up to `maxChunks` chunks off the end of a file's storage
    ///         so it can shrink or be wiped for re-upload from scratch.
    ///         Deleting a large file's storage in one go - looping over
    ///         every chunk to zero it - can need more gas than fits in a
    ///         block; this bounds that work per call the same way setChunk
    ///         already spreads a big upload across many transactions. Call
    ///         repeatedly until fileChunkCount(path) reaches the desired
    ///         length (0 to fully wipe it, or a lower target to just shrink
    ///         it before overwriting the surviving chunks with new
    ///         content). A no-op once there's nothing left to pop, so it's
    ///         safe to call one time too many.
    ///         Unregisters the path (404s it) on every call, not just the
    ///         first - a file that's only partway shrunk, or has been
    ///         shrunk but not yet overwritten with the new content it was
    ///         cleared to make room for, must never be served.
    function clearFile(string calldata path, uint256 maxChunks) external onlyOwner {
        registered[path] = false;
        bytes[] storage c = chunks[path];
        uint256 n = maxChunks < c.length ? maxChunks : c.length;
        for (uint256 i; i < n; ++i) {
            c.pop();
        }
    }
